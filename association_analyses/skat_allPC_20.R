...
bim <- read_bim("/datacommons/ochoalab/ssns_gwas/nephrotic.syndrome.gwas_proprocessing_202205/allele_freq/newfiles_alpha/imputation_10_new/results/GMMAT/ssns_ctr_hwe_mac20.bim")
bim_clean = bim %>% select(Chr = chr, Otherinfo5 = pos, Alt = alt, Ref = ref, id)
gene_set = read.table("/datacommons/ochoalab/ssns_gwas/nephrotic.syndrome.gwas_proprocessing_202205/allele_freq/newfiles_alpha/imputation_10_new/results/GMMAT/ssns_ctr/mac_20/SKAT/annovar/gene_group.txt", sep = " ", header = TRUE)
...
group.file <- "/datacommons/ochoalab/ssns_gwas/nephrotic.syndrome.gwas_proprocessing_202205/allele_freq/newfiles_alpha/imputation_10_new/results/GMMAT/ssns_ctr/mac_20/SKAT/group_file_allPC20.txt"
...
# prep model equation:
name <- "/datacommons/ochoalab/ssns_gwas/nephrotic.syndrome.gwas_proprocessing_202205/allele_freq/newfiles_alpha/imputation_10_new/results/case_control/split_filter/ssns_ctr.grm"
...
geno.file <- "/datacommons/ochoalab/ssns_gwas/nephrotic.syndrome.gwas_proprocessing_202205/allele_freq/newfiles_alpha/imputation_10_new/results/GMMAT/ssns_ctr/mac_20/SKAT/allssns_mac20_seq.gds"

print("start SMMAT")
# SMMAT
out <- SMMAT(model0, group.file = group.file,  geno.file = geno.file, MAF.range = c(1e-7, 0.5), miss.cutoff = 1, method = "davies", tests = "S", use.minor.allele = TRUE)
write.table(out, "SMMAT_out/allssns_PC20_smmat_sameweight.txt", row.names=FALSE, quote=FALSE, col.names=FALSE)
print("SMMAT done")
