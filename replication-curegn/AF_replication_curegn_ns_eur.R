...
write.table(gnomad_snp, paste0(dir, "AF/gnomad_snp_id_eur.txt"),
...
write.table(curegn_snps, paste0(dir, "AF/replication_unclump_curegn_snps_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
### clump Bristol p-values to get effective number of tests
clump_curegn <- read.table(paste0(dir,  "AF/ns/eur/clump_curegn_1437_eur.clumped"), header = TRUE) 
nrow(clump_curegn)
...
ancestry_all = read.table(paste0("/datacommons/ochoalab/ssns_gwas/imputed/eur/mac20-glmm-score.txt" ),  sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
...
ns_repli_table = cases_vs_control_test %>% filter(ID == "rs9274564")
...
write.table(SNP_nexus, paste0(dir,"AF/ns/eur/curegn_snp_nexus_allsig_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE, sep = "\t")

neargens = read.table(paste0(dir,"AF/ns/eur/near_gens_allsig.txt"), header = TRUE, sep = "\t") %>% select(-Chromosome, -Position)
gencoord = read.table(paste0(dir, "AF/ns/eur/gen_coords_allsig.txt"), header = TRUE, sep = "\t") %>% 
...
write.table(allsnps_merge_anno, paste0(dir, "AF/annotated_results/curegn_replication_ns_allsig_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
