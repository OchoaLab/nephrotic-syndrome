...
gnomad_AC <- read.table(paste0(dir, "gnomad/outfile/gnomad-genome_ssns_ctrl_eur_only.txt"), sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
gnomad_snp = gnomad_AC %>% mutate(SNP = paste0(CHROM, ":", POS)) %>% mutate(SNP = str_remove(SNP, "chr")) %>% select(SNP)
write.table(gnomad_snp, paste0(dir, "AF/ssns/eur/gnomad_snp_id_eur.txt"),
...
eur_ac <- read.table(paste0(dir, "AF/ssns/eur/curegnAC_eur_sub.acount")) 
...
write.table(output_plot, paste0(dir, "AF/annotated_results/replication_pvals_", disease_subtype,"_eur.txt"), 
...
hist(pvals_b, breaks = 50, main = "curegn SSNS vs gnomad controls: EUR")
qq(pvals_b, main = "curegn SSNS vs gnomad controls: EUR")
...
write.table(curegn_snps, paste0(dir, "AF/ssns/eur/replication_unclump_curegn_snps_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
### clump Bristol p-values to get effective number of tests
clump_curegn <- read.table(paste0(dir,  "AF/ssns/eur/clump_curegn_2347.clumped"), header = TRUE) 
nrow(clump_curegn)
...
ancestry_all = read.table(paste0("/datacommons/ochoalab/ssns_gwas/imputed/ssns_ctrl/eur/mac20-glmm-score.txt" ),  sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
...
cases_vs_control_pval %>% filter(ID == "rs138996")
...
write.table(SNP_nexus, paste0(dir,"AF/ssns/eur/curegn_snp_nexus_allsig_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE, sep = "\t")

neargens = read.table(paste0(dir,"AF/ssns/eur/near_gens_allsig.txt"), header = TRUE, sep = "\t") %>% select(-Chromosome, -Position)
gencoord = read.table(paste0(dir, "AF/ssns/eur/gen_coords_allsig.txt"), header = TRUE, sep = "\t") %>% 
...
all_snps = cases_vs_control_pval %>% filter(sig == TRUE) %>% 
  select(CHR, BP, A1 = REF, A2 = ALT, rsid = ID, CureGN_LRT_PVAL = P)
...
write.table(allsnps_merge_anno, paste0(dir, "AF/annotated_results/curegn_replication_ssns_allsig_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
