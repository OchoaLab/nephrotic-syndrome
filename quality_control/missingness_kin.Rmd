...
acq_num_intersect <- intersect(phenotype1$AcquisitionNumber, phenotype1_2$AcquisitionNumber)
# ensure that data agrees enough
tmp1 <- phenotype1[ match( acq_num_intersect, phenotype1$AcquisitionNumber ), ]
tmp2 <- phenotype1_2[ match( acq_num_intersect, phenotype1_2$AcquisitionNumber ), ]
# exact matches
stopifnot( all( tmp1$Sex == tmp2$Sex ) )
stopifnot( all( tmp1$RACE == tmp2$RACE ) )
# rest are good enough, all ages match except one has an extra NA (and one has a stupid "YRS" suffix), and 3 samples switched form SSNS to SRNS, but it's clear they're the same individuals
# remove repeated rows (info overlap, manually checked)
phenotype1 = phenotype1 %>% 
  filter( !( AcquisitionNumber %in% acq_num_intersect ) )
...
# check for overlap ID's
acq_num_intersect <- intersect(phenotype1_$AcquisitionNumber, phenotype2$`Acquisition Number`)
tmp1 <- phenotype1_[ match( acq_num_intersect, phenotype1_$AcquisitionNumber ), ]
tmp2 <- phenotype2[ match( acq_num_intersect, phenotype2$`Acquisition Number` ), ]
# exact matches
stopifnot( all( tmp1$Sex == tmp2$SEX_CDE ) )
stopifnot( all( tmp1$RACE == tmp2$`Broad Race` ) )
# remove repeated rows (information overlap from both phenotype files)
phenotype2 = phenotype2 %>% 
  filter( !( `Acquisition Number` %in% acq_num_intersect ) ) %>% 
...
writeLines(paste0(list_remove, " ", list_remove), fileConn)
close(fileConn)
