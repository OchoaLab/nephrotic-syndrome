...
write.table(gnomad_snp, paste0(dir, "AF/ns/gnomad_snp_id.txt"),
            row.names=FALSE, quote=FALSE, col.names = FALSE)
cases_vs_control %>% filter(ID == "rs2856695")
...
sas_ac <- read.table(paste0(dir, "AF/ns/curegnAC_sas.acount")) 
...
sas_ac_clean  %>% filter(POS == "32684117")

eur_ac <- read.table(paste0(dir, "AF/ns/curegnAC_eur.acount")) 
...
afr_ac <- read.table(paste0(dir, "AF/ns/curegnAC_afr.acount")) 
...
eas_ac <- read.table(paste0(dir, "AF/ns/curegnAC_eas.acount")) 
...
write.table(merge3, paste0(dir, "AF/ns/curegn_AC_ns.txt"),
...
write.table(curegn_snps, paste0(dir, "AF/ns/replication_unclump_curegn_snps.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
### clump Bristol p-values to get effective number of tests
clump_curegn <- read.table(paste0(dir,  "AF/ns/clump_curegn_5629.clumped"), header = TRUE) 
nrow(clump_curegn)
...
ancestry_all = read.table("/datacommons/ochoalab/ssns_gwas/imputed/mac20-glmm-score.txt" ,  sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
...
write.table(SNP_nexus, paste0(dir,"AF/ns/curegn_snp_nexus_allsig.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE, sep = "\t")

neargens = read.table(paste0(dir,"AF/ns/near_gens_allsig.txt"), header = TRUE, sep = "\t") %>% select(-Chromosome, -Position)
gencoord = read.table(paste0(dir, "AF/ns/gen_coords_allsig.txt"), header = TRUE, sep = "\t") %>% 
...
write.table(allsnps_merge_anno, paste0(dir, "AF/annotated_results/curegn_replication_ns_allsig.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
