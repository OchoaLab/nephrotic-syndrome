...
gnomad_AC <- read.table(paste0(dir, "gnomad/outfile_new/gnomad-genome_", disease_subtype, "_", ancestry, ".txt"), sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE) %>% filter(AC_sas != 0)
...
sas_ac <- read.table(paste0(dir, disease_subtype, "/", ancestry, "/discoveryAC_sas_gnomadsnps.acount")) 
...
write.table(sas_ac_clean, paste0(dir, disease_subtype, "/", ancestry, "/discovery_AC_sas.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE)

# step 4: Merge discovery AC and gnomad AC
discovery_AC <- read.table(paste0(dir, disease_subtype,  "/", ancestry, "/discovery_AC_sas.txt"), 
                           header = TRUE, sep = " ")
...
x1 <- AC_df$disc_AC_sas
n1 <- AC_df$disc_AN_sas
x2 <- AC_df$AC_sas
n2 <- AC_df$AN_sas
...
sas_ac <- read.table(paste0(dir, disease_subtype,  "/", ancestry, "/bristolAC_sas.acount")) 
colnames(sas_ac) = c("CHROM", "ID", "REF", "ALT", "ALT_FREQS", "OBS_CT")
sas_ac_clean = sas_ac  %>%
...
write.table(sas_ac_clean, 
            paste0(dir, disease_subtype,  "/", ancestry, "/bristol_AC_ns_sas.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE)

bristol_AC <- read.table(paste0(dir, disease_subtype,  "/", ancestry,  "/bristol_AC_ns_sas.txt"), header = TRUE) 
...
x1 <- cases_vs_control$bristol_AC_sas
n1 <- cases_vs_control$bristol_AN_sas
x2 <- cases_vs_control$AC_sas
n2 <- cases_vs_control$AN_sas
# p-values for forward alignment (only alignment for non-revcomp SNPs)
...
clump_bristol <- read.table(paste0(dir, disease_subtype, "/", ancestry, "/clump_bristol_1295.clumped"), header = TRUE) 
nrow(clump_bristol)
...
write.table(SNP_nexus, paste0(dir, disease_subtype, "/bristol_snp_nexus_allsig_sas.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE, sep = "\t")
