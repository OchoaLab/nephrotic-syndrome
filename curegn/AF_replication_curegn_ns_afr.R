...
gnomad_AC <- read.table(paste0(dir, "gnomad/outfile/gnomad-genome_ns_ctrl_afr_only.txt"), sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
gnomad_snp = gnomad_AC %>% mutate(SNP = paste0(CHROM, ":", POS)) %>% mutate(SNP = str_remove(SNP, "chr")) %>% select(SNP)
write.table(gnomad_snp, paste0(dir, "AF/gnomad_snp_id_afr.txt"),
            row.names=FALSE, quote=FALSE, col.names = FALSE)
gnomad_AC %>% filter(ID == "rs28366363")
...
afr_ac <- read.table(paste0(dir, "AF/ns/afr/curegnAC_afr_sub.acount")) 
...
write.table(curegn_snps, paste0(dir, "AF/ns/afr/replication_unclump_curegn_snps_afr.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
### clump Bristol p-values to get effective number of tests
clump_curegn <- read.table(paste0(dir,  "AF/ns/afr/clump_curegn_1201_afr.clumped"), header = TRUE) 
nrow(clump_curegn)
...
ancestry_all = read.table(paste0("/datacommons/ochoalab/ssns_gwas/imputed/afr/mac20-glmm-score.txt" ),  sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
...
ns_repli_table = cases_vs_control_test %>% filter(ID == "rs28366363")
...
write.table(SNP_nexus, paste0(dir,"AF/ns/afr/curegn_snp_nexus_allsig_afr.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE, sep = "\t")

neargens = read.table(paste0(dir,"AF/ns/afr/near_gens_allsig.txt"), header = TRUE, sep = "\t") %>% select(-Chromosome, -Position)
gencoord = read.table(paste0(dir, "AF/ns/afr/gen_coords_allsig.txt"), header = TRUE, sep = "\t") %>% 
...
write.table(allsnps_merge_anno, paste0(dir, "AF/annotated_results/curegn_replication_ns_allsig_afr.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
