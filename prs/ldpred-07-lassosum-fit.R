...
name_in <- paste0( base, '-ldpred2-lassosum' )
...
if ( file.exists( paste0( train, '/', file_params ) ) ) {
    ...
    setwd( train )
    params <- read_tsv( file_params, col_types = 'ddidddddi' )
} else {
    library(bigsnpr)
    library(genio)
...
## stopifnot( all( params$cor > 0, na.rm = TRUE ) ) # apparently this isn't true either
## stopifnot( all( params$cor_lower > 0, na.rm = TRUE ) ) # some of these are negative!  set to zero?
