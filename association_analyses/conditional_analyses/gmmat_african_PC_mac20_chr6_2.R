...
covarfile = read.table("/datacommons/ochoalab/ssns_gwas/GMMAT_0418/ssns_ctr/conditional/conditional2/afr/covar_chr6_2_ssns_control_afr.txt", sep = " ", stringsAsFactors=FALSE, quote = "", header = FALSE) 
...
pheno = merge(phenofile %>% rename(trait = V3), covarfile %>% rename(sex = V3, chr6 = V4, chr6_2 = V5), by = c("V1","V2")) 
pheno_all = merge(pheno, eigenfile, by = c("V1","V2")) %>% rename(famid = V1, id = V2)

model0 <- glmmkin(trait ~  sex + chr6 + chr6_2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12, data = pheno_all, kins = grm, id = "id",family = binomial(link = "logit"))
glmm.score(model0, infile = "/datacommons/ochoalab/ssns_gwas/GMMAT_0418/ssns_ctr/afr/ssns_ctr_afr_mac20", outfile ="glmm.score.bed.ancestry_black_ssns_ctr_mac20_PC_chr6_2.txt")
