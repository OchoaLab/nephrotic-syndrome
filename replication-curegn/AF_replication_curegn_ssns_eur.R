...
write.table(gnomad_snp, paste0(dir, "curegn/AF/ssns/eur/gnomad_snp_id.txt"),
...
write.table(curegn_snps, paste0(dir, "curegn/AF/ssns/eur/replication_unclump_curegn_snps_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
### clump p-values to get effective number of tests
clump_curegn <- read.table(paste0(dir,  "curegn/AF/ssns/eur/clump_curegn_2176.clumped"), header = TRUE) 
nrow(clump_curegn)
...
ancestry_all = read.table(paste0("/datacommons/ochoalab/ssns_gwas/saige/ssns_ctrl/eur/saige_output.txt" ),  sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
...
ssns_repli_table = cases_vs_control_test %>% filter(ID %in% c("rs281874807", "rs1694133"))
gnomad_AC %>% filter(ID == "rs1694133")
...
write.table(SNP_nexus, paste0(dir,"curegn/AF/ssns/eur/curegn_snp_nexus_allsig_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE, sep = "\t")

neargens = read.table(paste0(dir,"curegn/AF/ssns/eur/near_gens.txt"), header = TRUE, sep = "\t") %>% select(-Chromosome, -Position)
gencoord = read.table(paste0(dir, "curegn/AF/ssns/eur/gen_coords.txt"), header = TRUE, sep = "\t") %>% 
...
write.table(allsnps_merge_anno, paste0(dir, "curegn/AF/annotated_results/curegn_replication_ssns_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
