...
sas_ac <- read.table(paste0(dir, "bristol/AF/ns_ctrl/discoveryAC_sas_ukbbsnps.acount"))
...
eur_ac <- read.table(paste0(dir, "bristol/AF/ns_ctrl/discoveryAC_eur_ukbbsnps.acount"))
...
afr_ac <- read.table(paste0(dir, "bristol/AF/ns_ctrl/discoveryAC_afr_ukbbsnps.acount"))
...
write.table(merge2, 
            paste0(dir,  "bristol/AF/", disease_subtype, "/bristol_AC_ns_allage.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE)

bristol_AC <- read.table(paste0(dir, "bristol/AF/",  disease_subtype, "/bristol_AC_ns_allage.txt"), header = TRUE) %>% 
...
clump_bristol <- read.table(paste0(dir, "bristol/AF/", disease_subtype, "/clump_bristol_4290.clumped"), header = TRUE) 
nrow(clump_bristol)
...
snp_list = c("6:32652506", "10:28810849", "16:11071160", "5:152964673", "6:32689478", "6:31504943")
ns_repli_table = cases_vs_control_test %>% filter(SNP %in% snp_list)

bristol_AC %>% filter(CHROM == 6 & POS == 32652506)
bristol_AC %>% filter(CHROM == 10 & POS == 28810849)
bristol_AC %>% filter(CHROM == 6 & POS == 32689478)