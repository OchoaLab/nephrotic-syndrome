...
sas_ac <- read.table(paste0(dir, "bristol/AF/ns_ctrl/sas/discoveryAC_sas_ukbbsnps.acount"))
...
write.table(sas_ac_clean, 
            paste0(dir,  "bristol/AF/", disease_subtype, "/", ancestry, "/bristol_AC_ns_allage_sas.txt"), 
...
clump_bristol <- read.table(paste0(dir, "bristol/AF/", disease_subtype, "/", ancestry, "/clump_bristol_928.clumped"), header = TRUE) 
nrow(clump_bristol)
...
snp_list = c("6:32604403", "10:28810849")
ns_repli_table = cases_vs_control_test %>% filter(SNP %in% snp_list)

bristol_AC %>% filter(CHROM == 6 & POS == 152964673)
