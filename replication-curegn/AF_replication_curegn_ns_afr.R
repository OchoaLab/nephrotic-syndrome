...
gnomad_AC <- read.table(paste0(dir, "gnomad/outfile/gnomad-genome_ns_ctrl_afr_clean.txt"), sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
gnomad_snp = gnomad_AC %>% mutate(SNP = paste0(CHROM, ":", POS)) %>% mutate(SNP = str_remove(SNP, "chr")) %>% select(SNP)
write.table(gnomad_snp, paste0(dir, "curegn/AF/ns/afr/gnomad_snp_id.txt"),
...
afr_ac <- read.table(paste0(dir, "curegn/AF/ns/afr/curegnAC_afr_sub.acount")) 
...
hist(pvals_b, breaks = 50, main = "curegn NS vs gnomad controls: AFR")
qq(pvals_b, main = "curegn NS vs gnomad controls: AFR")
...
write.table(curegn_snps, paste0(dir, "curegn/AF/ns/afr/replication_unclump_curegn_snps_afr.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
### clump p-values to get effective number of tests
clump_curegn <- read.table(paste0(dir,  "curegn/AF/ns/afr/clump_curegn_1105.clumped"), header = TRUE) 
nrow(clump_curegn)
...
ancestry_all = read.table(paste0("/datacommons/ochoalab/ssns_gwas/saige/ns_ctrl/afr/saige_output.txt" ),  sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
...
ns_repli_table = cases_vs_control_test %>% filter(ID == "rs57588792")
...
write.table(SNP_nexus, paste0(dir,"curegn/AF/ns/afr/curegn_snp_nexus_allsig_afr.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE, sep = "\t")

neargens = read.table(paste0(dir,"curegn/AF/ns/afr/near_gens.txt"), header = TRUE, sep = "\t") %>% select(-Chromosome, -Position)
gencoord = read.table(paste0(dir, "curegn/AF/ns/afr/gen_coords.txt"), header = TRUE, sep = "\t") %>% 
...
write.table(allsnps_merge_anno, paste0(dir, "curegn/AF/annotated_results/curegn_replication_ns_allsig_afr.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
