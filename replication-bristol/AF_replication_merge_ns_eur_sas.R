...
write.table(merge1, paste0(dir, disease_subtype, "/discovery_AC_ancestry_combined_eur_sas.txt"),
            row.names=FALSE, quote=FALSE, col.names = TRUE)

# step 4: Merge discovery AC and gnomad AC
discovery_AC <- read.table(paste0(dir, disease_subtype, "/discovery_AC_ancestry_combined_eur_sas.txt"), 
                           header = TRUE, sep = " ")
AC_df = merge(gnomad_AC, discovery_AC, by = c("CHROM", "POS", "REF", "ALT"), all = TRUE)
...
write.table(control_snp,
            "/datacommons/ochoalab/ssns_gwas/replication/bristol_data/AF/ns_ctrl/control_snp_id_all_eur_sas.txt", 
            row.names=FALSE, quote=FALSE, col.names = FALSE)
...
write.table(merge1, 
            paste0(dir, disease_subtype, "/bristol_AC_ns_allage_allsnps_eur_sas.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE)

bristol_AC <- read.table(paste0(dir, disease_subtype, "/bristol_AC_ns_allage_allsnps_eur_sas.txt"), header = TRUE) 
...
clump_bristol <- read.table(paste0(dir, disease_subtype, "/clump_bristol_5259.clumped"), header = TRUE) 
nrow(clump_bristol)
...
write.table(SNP_nexus, paste0(dir, disease_subtype, "/bristol_snp_nexus.txt"), 
...
ancestry_all = read.table(paste0("/datacommons/ochoalab/ssns_gwas/imputed/", "/mac20-glmm-score.txt" ),  sep = "\t", stringsAsFactors=FALSE, quote = "", header = TRUE)
...
write.table(ind_reg_anno, paste0(dir, "/annotated_results/bristol_replication_", disease_subtype, ".txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE, sep = "\t")
...
ns_repli_table = cases_vs_control_test %>% filter(ID %in% rsid_list)
