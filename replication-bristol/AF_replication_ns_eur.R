...
ukbb_AC <- read.csv(paste0(dir, "ukbb/output_file/UKBB_ns_ctrl_eur.csv"))
...
eur_ac <- read.table(paste0(dir, "bristol/AF/ns_ctrl/eur/discoveryAC_eur_ukbbsnps.acount"))
...
control_vs_control = merge(ukbb_AC %>% mutate(CHROM = paste0("chr", Chromosome)) %>% 
  ...
  select(CHROM, POS, A1, A2, rsid, ukbb_AC_eur, ukbb_AN_eur, 
         disc_AC_eur, disc_AN_eur) %>% distinct()
...
x1 <- cbind(control_vs_control$disc_AC_eur)
n1 <- cbind(control_vs_control$disc_AN_eur)
x2 <- cbind(control_vs_control$ukbb_AC_eur)
n2 <- cbind(control_vs_control$ukbb_AN_eur)
...
hist(pvals, breaks = 50, main = "discovery control vs ukbb control (eur)")
...
write.table(control_snp, paste0(dir, "bristol/AF/", disease_subtype, "/", ancestry,  "/clean_control_snp_id.txt"), 
            row.names=FALSE, quote=FALSE, col.names = FALSE)

# run allele_freq_bristol.q, process outfiles
eur_ac <- read.table(paste0(dir, "bristol/AF/", disease_subtype, "/", ancestry,  "/bristolAC_eur.acount")) 
colnames(eur_ac) = c("CHROM", "ID", "REF", "ALT", "ALT_FREQS", "OBS_CT")
eur_ac_clean = eur_ac  %>%
...
write.table(eur_ac_clean, 
            paste0(dir,  "bristol/AF/", disease_subtype, "/", ancestry, "/bristol_AC_ns_allage_eur.txt"), 
            row.names=FALSE, quote=FALSE, col.names = TRUE)

bristol_AC <- read.table(paste0(dir, "bristol/AF/",  disease_subtype,  "/", ancestry,  "/bristol_AC_ns_allage_eur.txt"), header = TRUE) %>% 
...
cases_vs_control = merge(ukbb_AC, bristol_AC, by.x = c("Chromosome", "Position", "A1", "A2"), 
  ...
  select(Chromosome, Position, A1, A2,   
         ukbb_AC_eur, ukbb_AN_eur,
         bristol_AC_eur, bristol_AN_eur) %>% distinct()

# LRT on Bristol vs UKBB (case vs control)
# define function input
# x1/n1: bistrol; x2/n2: UKBB
x1 <- cbind(cases_vs_control$bristol_AC_eur)
n1 <- cbind(cases_vs_control$bristol_AN_eur)
x2 <- cbind(cases_vs_control$ukbb_AC_eur)
n2 <- cbind(cases_vs_control$ukbb_AN_eur)
# p-values for forward alignment (only alignment for non-revcomp SNPs)
...
clump_bristol <- read.table(paste0(dir, "bristol/AF/", disease_subtype, "/", ancestry, "/clump_bristol_1164.clumped"), header = TRUE) 
nrow(clump_bristol)
...
snp_list = c("6:32652455", "10:28810849", "16:11071160", "5:152964673", "6:32689478", "6:31504943")
ns_repli_table = cases_vs_control_test %>% filter(SNP %in% snp_list)

bristol_AC %>% filter(CHROM == 5 & POS == 152964673)
