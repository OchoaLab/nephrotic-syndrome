...
data_comp <- tibble(
    TGP  = c(  tgp_afr$ALT_FREQS[ indexes_comp ],  tgp_eur$ALT_FREQS[ indexes_comp ],  tgp_sas$ALT_FREQS[ indexes_comp ] ),
    SSNS = c( ssns_afr$ALT_FREQS[ indexes_comp ], ssns_eur$ALT_FREQS[ indexes_comp ], ssns_sas$ALT_FREQS[ indexes_comp ] ),
...
data_noncomp <- tibble(
    TGP  = c(  tgp_afr$ALT_FREQS,  tgp_eur$ALT_FREQS,  tgp_sas$ALT_FREQS ),
    SSNS = c( ssns_afr$ALT_FREQS, ssns_eur$ALT_FREQS, ssns_sas$ALT_FREQS ),
...
# this plots "Unchanged" first (at back), followed by "Flip", "Remove", in order of Category factor, so we can see smaller categories most clearly
...
ggplot( data, aes( x = SSNS, y = TGP, color = Category ) ) +
    theme_classic() +
    xlim( 0, 1 ) +
    ylim( 0, 1 ) +
    geom_point(size = 0.5) + 
    scale_color_manual(values=c("#999999", "#56B4E9",  "darkgreen", "yellow", "red")) +
